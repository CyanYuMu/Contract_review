...
export function getElementListByHTML(
  ...
  const processedNodes = new WeakSet<Node>() // 防止无限递归
  function findTextNode(dom: Element | Node) {
    // 防止重复处理同一个节点
    if (processedNodes.has(dom)) {
      return
    }
    ...
    try {
          ...
          try {
            ...
            if (node.nodeName === 'BR') {
              ...
              try {
                ...
                console.warn('处理标题元素时出错，跳过:', error)
                // 如果处理失败，尝试提取纯文本
                const text = hElement.textContent || hElement.innerText || ''
                if (text.trim()) {
                  elementList.push({
                    value: text + '\n',
                    type: ElementType.TITLE,
              ...
              if (element.trList!.length) {
                ...
                // 如果第一行没有单元格，使用最大列数
                const tdCount = element.trList![0].tdList.length > 0
                  ? element.trList![0].tdList.reduce(
                      (pre, cur) => pre + (cur.colspan || 1),
                      0
                    )
                  : maxColCount
                
                const defaultWidth = Math.ceil(options.innerWidth / Math.max(tdCount, maxColCount, 1))
                const actualColCount = Math.max(tdCount, maxColCount, 1)
              ...
              try {
                ...
                console.warn('处理节点时出错，跳过:', node.nodeName, error)
                // 如果处理失败，尝试提取纯文本
                if (node.nodeType === 1) {
                  const text = (node as Element).textContent || ''
                  if (text.trim()) {
                    elementList.push({ value: text + '\n' })
                  }
                }
              }
            }
      ...
      console.warn('处理 DOM 节点时出错:', error)
    }
  }
  // 追加dom
  const clipboardDom = document.createElement('div')
  try {
    ...
    // 如果 HTML 解析失败，尝试作为纯文本处理
    const text = htmlText.replace(/<[^>]*>/g, '') // 移除所有 HTML 标签
    if (text.trim()) {
      text.split('\n').forEach(line => {
        if (line.trim()) {
          elementList.push({ value: line + '\n' })
        }
      })
    }
    return elementList
  ...
  try {
    ...
    console.error('处理 DOM 节点时出错:', error)
    // 如果处理失败，尝试提取纯文本
    const text = clipboardDom.textContent || clipboardDom.innerText || ''
    if (text.trim()) {
      text.split('\n').forEach(line => {
        if (line.trim()) {
          elementList.push({ value: line + '\n' })
    ...
    try {
      ...
      // 如果移除失败，尝试从 body 中移除
      if (clipboardDom.parentNode) {
        clipboardDom.parentNode.removeChild(clipboardDom)
      }
    }
  }
  return elementList
...
export function replaceHTMLElementTag(
  ...
  for (let i = 0; i < oldDom.attributes.length; i++) {
    ...
    // 验证属性名是否有效（只允许字母、数字、连字符、下划线、冒号）
    // HTML 属性名规范：必须以字母开头，可以包含字母、数字、连字符、下划线、冒号
    if (attr.name && /^[a-zA-Z][a-zA-Z0-9\-_:]*$/.test(attr.name)) {
      try {
        newDom.setAttribute(attr.name, attr.value)
      } catch (error) {
        // 如果设置属性失败，跳过该属性
        console.warn(`跳过无效属性: ${attr.name}`, error)
      }
    } else {
      // 跳过无效的属性名
      console.warn(`跳过无效属性名: ${attr.name}`)
    }
  }
