    ...
    for (let t = 0; t < trList.length; t++) {
      ...
      for (let d = 0; d < tr.tdList.length; d++) {
          ...
          for (let c = start; c < colgroup.length; c++) {
            ...
            if (rowCount === t) {
              ...
              for (let preC = 0; preC < c; preC++) {
                if (preC < colgroup.length && colgroup[preC]?.width) {
        ...
        for (let col = 0; col < td.colspan; col++) {
          ...
          if (colIdx < colgroup.length && colgroup[colIdx]?.width) {
            width += colgroup[colIdx].width
          } else {
            // 如果 colgroup 中没有对应的列，使用默认宽度
            const defaultWidth = colgroup.length > 0 
              ? colgroup[0].width 
              : Math.ceil(this.draw.getInnerWidth() / (colgroup.length || 1))
            width += defaultWidth
          }
        }
        let height = 0
