...
func GetSuffix(u string, withDot bool) (suffix string) {
	if u == "" {
		return ""
	}
	left := strings.LastIndex(u, "/")
	if left == -1 {
		left = 0
	}
	right := strings.LastIndex(u, "?")
	if right == -1 {
		right = len(u)
	}
	s := u[left+1 : right]
	sepPos := strings.LastIndex(s, ".")
	if sepPos == -1 {
		return ""
	} else {
		if withDot {
...
func GetNameAndSuffix(u string, withSuffixDot bool) (baseName, suffix string) {
	...
	if sepPos == -1 {
		...
		baseName = s[0:sepPos]
		if withSuffixDot {
			return baseName, s[sepPos:]
		}
		return baseName, s[sepPos+1:]
	}
}

var typeRefer = map[string]string{
	".load":        "text/html",
