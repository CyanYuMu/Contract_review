...
* @return day
 */
func MilliTimeToYMD(timeMs int64) (year, month, day int) {
	t := time.Unix(timeMs/1000, 0)
	year = t.Year()
	month = int(t.Month())
	day = t.Day()
...
/*GetFewDaysAgoTimestamp
* @Description: 获取几天前的凌晨 00:00:00 的数据
* @param day
* @return int64
 */
func GetFewDaysAgoEarlyMorningTimestamp(day int) int64 {
	// 计算7天前的时间
	ago := time.Now().AddDate(0, 0, -day)
	// 设置时间为凌晨1点
	ago = time.Date(ago.Year(), ago.Month(), ago.Day(), 1, 0, 0, 0, ago.Location())

	// 获取时间戳
	return ago.UnixMilli()
}
...
// 获取距离当前时间的下一个能被n整除的时间点
func GetNextDivisibleTime(t time.Time, n int) time.Time {
	currentUnix := t.Unix()
	nextDivisibleUnix := ((currentUnix / int64(n)) + 1) * int64(n)
	return time.Unix(nextDivisibleUnix, 0)
}

// 获取距离当前时间的上一个能被n整除的时间点
func GetPreviousDivisibleTime(t time.Time, n int) time.Time {
	currentUnix := t.Unix()
	previousDivisibleUnix := (currentUnix / int64(n)) * int64(n)
	return time.Unix(previousDivisibleUnix, 0)
}

// 获取距离当前时间到当日的结束时间的时间差
func GetDayLeftDuration(t time.Time) time.Duration {
	endOfDay := GetEndOfDay(t)
	return endOfDay.Sub(t)
}

// ConvertToTimezone converts current time to the specified timezone
// Example timezone: "Asia/Shanghai", "America/New_York", "UTC"
