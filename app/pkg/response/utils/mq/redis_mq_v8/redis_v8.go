...
func (c *Consumer) cleanIdleConsumers(ctx context.Context, topic, group string) error {
	...
	for _, consumer := range consumers {
		if consumer.Idle > c.config.ConsumerMaxIdleTime.Milliseconds() && consumer.Pending == 0 {
			c.cli.XGroupDelConsumer(ctx, topic, group, consumer.Name)
			su_logger.Info(ctx, "clean idle consumer", su_logger.E().String("consumer", consumer.Name))
		}
	}
	return nil
}
...
func (c *Consumer) Consume(ctx context.Context, handler mq.MsgHandler) error {
	// 消费者ID，使用时间戳确保唯一性
	consumerID := fmt.Sprintf("%d", time.Now().UnixNano())
...
func retryWithExponentialBackoff(
	...
	for retryCount < maxRetries {
		...
		su_logger.Warn(ctx, "Redis操作失败，将进行重试",
			su_logger.E().
	...
	// 达到最大重试次数后返回最后一次错误
	return fmt.Errorf("达到最大重试次数(%d)后操作仍然失败: %w", maxRetries, err)
}
