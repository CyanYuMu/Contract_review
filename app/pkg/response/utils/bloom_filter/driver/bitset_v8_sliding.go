...
var luaSlidingExistsCheckScriptV8 = redis.NewScript(bloomFilterSlidingExistsCheckScript)
var luaSlidingMSExistsCheckScriptV8 = redis.NewScript(bloomFilterSlidingMSExistsCheckScript)

// BitsetV8Sliding 滑动布隆过滤器
// 使用多个时间窗口来追踪元素，自动清理过期数据
type BitsetV8Sliding struct {
	SlidingWindowBase
	redis redis.UniversalClient
}

// BitsetV8SlidingConf 滑动布隆过滤器配置
type BitsetV8SlidingConf struct {
	// redis cli 实例
	Redis redis.UniversalClient
	// 容量
	Cap uint
...
func NewBitsetV8Sliding(conf *BitsetV8SlidingConf) (*BitsetV8Sliding, error) {
...
func (b *BitsetV8Sliding) getLocation(item string) []interface{} {
	return CalculateBloomFilterLocationsWithInterface(b.cap, b.p, item)
}

// Exists 检查元素是否存在于任何活动窗口中
func (b *BitsetV8Sliding) Exists(ctx context.Context, key string, item string) (exists bool, err error) {
	// 先更新窗口
	...
	// 使用 Lua 脚本一次性检查所有窗口
	rs := luaSlidingExistsCheckScriptV8.Eval(ctx, b.redis, windowKeys, locations...)
	if rs.Err() != nil {
		return false, rs.Err()
	}
...
// MExists 批量检查元素是否存在
func (b *BitsetV8Sliding) MExists(ctx context.Context, key string, items []string) (statusList []bool, err error) {
	if len(items) == 0 {
		return []bool{}, nil
	}
	...
	rs := luaSlidingMSExistsCheckScriptV8.Eval(ctx, b.redis, windowKeys, locations...)
	if rs.Err() != nil {
		return nil, rs.Err()
	}

	resultSlice, err := rs.Slice()
	if err != nil {
		return nil, err
	}
...
// Add 添加元素到当前窗口
func (b *BitsetV8Sliding) Add(ctx context.Context, key string, item string, ttl time.Duration) (ok bool, err error) {
	redisDel := func(ctx context.Context, key string) {
		_ = b.redis.Del(ctx, key)
	}
	...
	rs := luaAddScriptV8.Eval(ctx, b.redis, []string{windowKey}, locations...)
	if rs.Err() != nil {
		return false, rs.Err()
	}
...
// MAdd 批量添加元素
func (b *BitsetV8Sliding) MAdd(ctx context.Context, key string, items []string, ttl time.Duration) (statusList []bool, err error) {
	statusList = make([]bool, 0, len(items))
	for _, item := range items {
...
// Info 返回布隆过滤器的信息
func (b *BitsetV8Sliding) Info(ctx context.Context, key string) (info map[string]interface{}, err error) {
	redisDel := func(ctx context.Context, key string) {
		_ = b.redis.Del(ctx, key)
	}
	_, err = getCurrentWindow(ctx, &b.SlidingWindowBase, redisDel, key)
...
func (b *BitsetV8Sliding) Insert(ctx context.Context, key string, item string, ttl time.Duration) (ok bool, err error) {
	redisDel := func(ctx context.Context, key string) {
		_ = b.redis.Del(ctx, key)
	}
	currentIdx, err := getCurrentWindow(ctx, &b.SlidingWindowBase, redisDel, key)
	...
	rs := luaInsertScriptV8.Eval(ctx, b.redis, []string{windowKey}, locations...)
	if rs.Err() != nil {
		return false, rs.Err()
	}
...
// MInsert 批量插入元素
func (b *BitsetV8Sliding) MInsert(ctx context.Context, key string, items []string, ttl time.Duration) (statusList []bool, err error) {
	statusList = make([]bool, 0, len(items))
	for _, item := range items {
...
func (b *BitsetV8Sliding) Clear(ctx context.Context, key string) (err error) {
	// 删除所有窗口的Redis key
