...
type DistributedSentinel struct {
	baseLimiter
	redis redis.UniversalClient
}

func NewDistributedLimiter() *DistributedSentinelV8 {
	return &DistributedSentinelV8{}
}

func (d *DistributedSentinel) Init(cnf *Config) error {
	//TODO implement me
	if cnf.Redis == nil {
...
type SourceTaker func(input interface{}, key string) string

func (d *DistributedSentinel) Take(ctx context.Context, uri string, source string) (entry *Entry, err error) {
	//TODO implement me
...
// TryAcquire
// @description 尝试获取令牌
func (d *DistributedSentinel) tryAcquire(ctx context.Context, key string, rule *uriGroup) (ok bool, err error) {
	//TODO implement me
	args := []interface{}{rule.Burst, rule.PeriodSec, 1}
	rs, err := redisScript.Run(ctx, d.redis, []string{key}, args...).Int()
